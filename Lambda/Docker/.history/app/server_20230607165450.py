...
def create_app():
    ...
    def login():
        ...
        if request.method == 'POST':
            ...
            # Crea un client SNS utilizzando le credenziali di accesso AWS
            if app.config['sns'] is None:
                app.config['sns'] = boto3.client('sns',
                            aws_access_key_id=app.config['ACCESS_KEY_ID'],
                            aws_secret_access_key=app.config['ACCESS_KEY'],
                            aws_session_token=app.config['SESSION_TOKEN'],
    ...
    def upload_file():
        ...
        if request.method == 'POST':
            ...
            URLTest = app.config['s3'].generate_presigned_url('get_object',
    ...
    def send_email(nome):
        ...
        # Definisci l'oggetto, il messaggio e l'ARN dell'endpoint di destinazione
        subject = 'Detected in a photo'
        message = 'You are detected in a photo on' + session.get('URL')
        if nome == 'Luca':
            endpoint_arn = 'arn:aws:sns:us-east-1:010324762728:PhotoMail:344906e5-e18a-455f-ac07-f1b0167ecb27'
        elif nome == 'Totto':
            endpoint_arn = 'ARN_DEL_ENDPOINT_DA_NOTIFICARE'
        
        if nome is not None:
            # Invia la notifica all'endpoint specificato
            response = app.config['sns'].publish(
                #TopicArn='arn:aws:sns:us-east-1:010324762728:PhotoMail',  # ARN del topic di SNS
                Subject=subject,
                Message=message,
                MessageAttributes={
                    'AWS.SNS.SMS.SenderID': {
                        'DataType': 'String'
                        # Identificatore personalizzato del mittente
                    }
                },
                TargetArn=endpoint_arn
            )

            # Verifica se l'invio della notifica è stato effettuato con successo
            if response['ResponseMetadata']['HTTPStatusCode'] == 200:
                print('Notifica inviata con successo all\'endpoint specificato.')
            else:
                print('Errore durante l\'invio della notifica:', response['ResponseMetadata']['HTTPStatusCode'])
