...
def insertInTable(aws_key, aws_key_id, aws_token, email, paswemail):
    # DYNAMODB
    dynamo = boto3.resource('dynamodb',endpoint_url='http://localhost:8000', region_name='us-east-1', aws_access_key_id='your-access-key-id',
                          aws_secret_access_key='your-secret-access-key')
    # Definisci il nome della tabella
    table_name = 'Credenziali'
    ...
    if table_name in existing_tables:
        ...
        table.update_item(
            Key={'ID': '1'},
            UpdateExpression='SET ACCESS_KEY_ID = :val',
            ExpressionAttributeValues={':val': aws_key_id}
        )
        table.update_item(
            Key={'ID': '2'},
            UpdateExpression='SET ACCESS_KEY = :val',
            ExpressionAttributeValues={':val': aws_key}
        )
        table.update_item(
            Key={'ID': '3'},
            UpdateExpression='SET SESSION_TOKEN = :val',
            ExpressionAttributeValues={':val': aws_token}
        )
        table.update_item(
            Key={'ID': '4'},
        ...
        table.update_item(
            Key={'ID': '5'},
            UpdateExpression='SET PASSWORD_EMAIL = :val',
            ExpressionAttributeValues={':val': paswemail}
        )
        ...
        with table.batch_writer() as batch:
            ...
            batch.put_item(Item={'ID': '3', 'SESSION_TOKEN': aws_token})
            batch.put_item(Item={'ID': '4', 'EMAIL': email})
            batch.put_item(Item={'ID': '5', 'PASSWORD_EMAIL': paswemail})
...
def login():
    ...
    if request.method == 'POST':
        ...
        insertInTable(app.config['ACCESS_KEY'], app.config['ACCESS_KEY_ID'], app.config['SESSION_TOKEN'], app.config['EMAIL'], app.config['PASSWEMAIL'])
