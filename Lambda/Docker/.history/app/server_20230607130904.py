...
class ServerRPCClass(object):
    ...
    def on_response(self, ch, method, props, body):
        if  self.corr_id == props.correlation_id:
            ...
            redis_store.get('s3').download_file(bucket_name, folder_name + 'test.jpg', os.path.join(app.config['EDITED_FOLDER'],'test.jpg'))
...
def create_app():
    ...
    def login():
        ...
        if request.method == 'POST':
            ...
            if authenticated:
                ...
                redis_store.set('ACCESS_KEY_ID', request.form['aws_accesskey_id'])
                redis_store.set('ACCESS_KEY', request.form['aws_accesskey'])
                redis_store.set('SESSION_TOKEN', request.form['aws_session_token'])
                redis_store.set('EMAIL', request.form['email_address'])
                redis_store.set('PASSWEMAIL', request.form['email_password'])
                ...
                if redis_store.get('s3') is None:
                    s3_client = boto3.client(
                        ...
                        aws_access_key_id=redis_store.get('ACCESS_KEY_ID'),
                        aws_secret_access_key=redis_store.get('ACCESS_KEY'),
                        aws_session_token=redis_store.get('SESSION_TOKEN')
                    )
                    redis_store.set('s3', s3_client)
    ...
    def upload_file():
        ...
        if request.method == 'POST':
            ...
            redis_store.get('s3').put_object(Bucket=bucket_name, Key=key, Body=file_data)

            signed_url = app.config['s3'].generate_presigned_url('get_object',
                        Params={'Bucket': bucket_name, 'Key': folder_name + 'test.jpg'},
                        ExpiresIn=3600)
            
            redis_store.set('signed_url', signed_url)

            #creo un JSON in cui inserire filename e credenziali di aws
            dati = {
                "filename": filename,
                "aws_accesskey_id": redis_store.get('ACCESS_KEY_ID'),
                "aws_accesskey": redis_store.get('ACCESS_KEY'),
                "aws_session_token": redis_store.get('SESSION_TOKEN')
            }
    ...
    def send_email(nome):
        ...
        if redis_store.get('signed_url') is not None:

            dati = {
                "nome": nome,
                "email_address": redis_store.get('EMAIL'),
                "email_password": redis_store.get('PASSWORD'),
                "url": redis_store.get('signed_url')
            }
        ...
        #         aws_session_token=app.config['SESSION_TOKEN']
        #     )
        #     app.config['signed_url'] = s3_client.generate_presigned_url('get_object', 
        #             Params={'Bucket': bucket_name, 'Key': folder_name + 'test.jpg'},
        #             ExpiresIn=3600)
        #     dati = {
        #         "nome": nome,
