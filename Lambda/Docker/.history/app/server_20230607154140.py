...
def create_app():
        ...
        if request.method == 'POST':
            ...
            if session['s3'] is None:
                session['s3'] = boto3.client(
                    ...
                    aws_secret_access_key=session['ACCESS_KEY'],
    ...
    def upload_file():
        ...
        if request.method == 'POST':
            ...
            session['s3'].put_object(Bucket=bucket_name, Key=key, Body=file_data)
            ...
            #             ExpiresIn=3600)
            URL = session['s3'].generate_presigned_url('get_object',
                        Params={'Bucket': bucket_name, 'Key': folder_name + 'test.jpg'},
                        ExpiresIn=3600)
