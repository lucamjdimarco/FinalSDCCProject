...
class ServerRPCClass(object):
    ...
    def on_response(self, ch, method, props, body):
        if  self.corr_id == props.correlation_id:
            ...
            app.config['s3'].download_file(bucket_name, folder_user + 'test.jpg', os.path.join(app.config['EDITED_FOLDER'],'test.jpg'))
...
def cleanup():
    ...
    if app.config['s3'] != None:
        response = app.config['s3'].list_objects_v2(
            Bucket=bucket_name,
            Prefix=folder_user
        )
        ...
        if 'Contents' in response:
            ...
            app.config['s3'].delete_objects(
                Bucket=bucket_name,
                Delete={'Objects': objects}
        )
            
        app.config['s3'].delete_object(
            Bucket=bucket_name,
            Key=folder_user
        )
...
def login():
    ...
    if request.method == 'POST':
        ...
        folder_user = folder_name + app.config['ACCESS_KEY_ID'] + '/'

        try:
            response = app.config['s3'].list_objects(
                Bucket=bucket_name,
                Prefix=folder_user
            )
        ...
        if 'Contents' not in response:
            ...
            app.config['s3'].put_object(Bucket=bucket_name, Key=folder_user)
        else:
            # Cancella i file presenti nella cartella s3
            objects = [{'Key': obj['Key']} for obj in response['Contents']]
            try:
                app.config['s3'].delete_objects(
                    Bucket=bucket_name,
                ...
                print(e)
                return redirect(url_for('login'))
            #Elimina tutto ciò che è contenuto nella cartella EDITED
